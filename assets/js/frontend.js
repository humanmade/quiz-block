...
( function () {
	...
	function initQuiz( quizEl ) {
		let questions = Array.prototype.slice.call( quizEl.querySelectorAll( '[data-question]' ) );
		...
		let customComplete = quizEl.querySelector( '[data-quiz-complete]' );
		if ( customComplete ) {
			customComplete.style.display = 'none';
		}

		// ── Progress bar ─────────────────────────────────────────────────────
		let progressEl = makeEl( 'div', { className: 'hmquiz__progress' },
			'<div class="hmquiz__progress-bar" role="progressbar" aria-valuenow="1" aria-valuemin="1" aria-valuemax="' + total + '">' +
			'<div class="hmquiz__progress-fill" style="width:0%"></div>' +
			'</div>' +
			'<p class="hmquiz__progress-text">Question <strong class="hmquiz__current-num">1</strong> of <strong>' + total + '</strong></p>'
		);
		quizEl.insertBefore( progressEl, quizEl.firstChild );

		let progressFill = progressEl.querySelector( '.hmquiz__progress-fill' );
		let progressBar  = progressEl.querySelector( '.hmquiz__progress-bar' );
		let currentNumEl = progressEl.querySelector( '.hmquiz__current-num' );
		...
		questions.forEach( function ( questionEl, index ) {
			...
			Array.prototype.slice.call( questionEl.children ).forEach( function ( child ) {
				if (
			...
			Array.prototype.slice.call( questionEl.querySelectorAll( '[data-feedback]' ) )
				.forEach( function ( fb ) {
			...
			// Alternative click.
			Array.prototype.slice.call( questionEl.querySelectorAll( '.hmquiz__alternative' ) )
				.forEach( function ( altBtn ) {
					altBtn.addEventListener( 'click', function () {
						if ( questionEl.getAttribute( 'data-answered' ) ) return;
						questionEl.setAttribute( 'data-answered', '1' );

						let correctId  = questionEl.getAttribute( 'data-correct' );
						let selectedId = altBtn.getAttribute( 'data-id' );
						...
						Array.prototype.slice.call( questionEl.querySelectorAll( '.hmquiz__alternative' ) )
							.forEach( function ( btn ) {
								...
								if ( btn.getAttribute( 'data-id' ) === correctId ) {
									btn.classList.add( 'hmquiz__alternative--correct' );
								} else if ( btn.getAttribute( 'data-id' ) === selectedId ) {
									btn.classList.add( 'hmquiz__alternative--incorrect' );
								}
							} );

						// Reveal feedback.
						let feedbackType = isCorrect ? 'correct' : 'incorrect';
						let feedbackEl   = questionEl.querySelector( '[data-feedback="' + feedbackType + '"]' );
						if ( feedbackEl ) {
							feedbackEl.style.display = '';
							feedbackEl.classList.add( 'hmquiz__feedback--visible' );
						}
			...
			nextBtn.addEventListener( 'click', function () {
				if ( ! isLast ) {
					questionEl.style.display = 'none';
					current++;
					let nextQ = questions[ current ];
					nextQ.style.display = '';
					nextQ.classList.add( 'hmquiz__question--enter' );
					updateProgress( progressFill, progressBar, currentNumEl, current, total );
					quizEl.scrollIntoView( {
						behavior: 'smooth',
						block: 'start',
					} );
				} else {
					showCompletion( quizEl, questions, progressFill, progressBar, customComplete );
				}
			} );
		} );
	...
	function showCompletion( quizEl, questions, fill, bar, customComplete ) {
		...
		if ( bar )  bar.setAttribute( 'aria-valuenow', questions.length );

		if ( customComplete ) {
			// Use the editor-defined completion screen if it's enabled.
			if ( customComplete.getAttribute( 'data-enabled' ) !== 'false' ) {
				let bg = customComplete.getAttribute( 'data-bg' );
				if ( bg ) customComplete.style.backgroundColor = bg;
			...
			let fallback = makeEl( 'div', { className: 'hmquiz__completion' },
				'<div class="hmquiz__completion-icon" aria-hidden="true">🎉</div>' +
				'<h2 class="hmquiz__completion-title">Quiz Complete!</h2>' +
				'<p class="hmquiz__completion-message">You answered all ' + questions.length + ' question' + ( questions.length !== 1 ? 's' : '' ) + '.</p>'
			);
